...
- To execute the experiments, the defining [`doe.py`](/experiments/dane/SO_C/doe.py) needs to exist. If so, the script [`run.sh`](/workflow/run.sh) executes the specified experiments based on the existing [experiments](/experiments/) as shown in Fig. 6 below.
