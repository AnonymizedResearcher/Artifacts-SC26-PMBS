...
system = {
    # 'queue': 'pdebug',
    'queue': 'pbatch',
    'slurm_time_limit': '00:06:00',  
    'flux_time_limit': '6m',  # on a flux managed system something like 0.5h=30m=1800s, 
    # 2m enough for B problem_class, but might be to short for C (A~1S, B~5s, C~10s )
    'sys_nodes': 1,
...
instance_level_1 = {
    ...
    'reservation_depth': [64], # 64 is the default, but setting it shows it in the config   
    # 'n_jobs': [20],
    # 'workload': ['sleep','babelstream','raja-perf%Polybench_GEMM%Base_HIP'],
    # # 'workload': ['sleep'],
    ...
    # 'job_request_cores_per_task': [24], # What a job requests in terms of resources, is needed for the flux bulksubmit for each job, as well as for the flux alloc instantiation
...
instance_level_2 = {  
    ...
    'reservation_depth': [64], # 64 is the default, but setting it shows it in the config
    'n_jobs': [40],
    'workload': ['babelstream','raja-perf%Polybench_GEMM%Base_HIP'],
